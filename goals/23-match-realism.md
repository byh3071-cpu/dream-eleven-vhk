...
## Phase F (DONE) — 재관전 폴리싱: 진동/공 훅/공 크기
사용자 재보고: 3D 부자연스러움("부르르 떨림", 공이 훅 감, 공 크게). Playwright 좌표 로깅으로
진동 정량화(선수당 방향 반전) + ablation.
- **부르르 떨림 = 복합(계측으로 이산 트리거 격리)**: ablation(separation 끄기)으로 반전
  9.5→4.8 확인. 계측(top-3 진동 선수 로깅)으로 outlier의 실체 규명:
  ① separation 톱니 진동 — current 직접 수정이 velocity와 불일치(당김 vs 밀기 톱니).
     → 2패스 리팩터로 목표(target)에 **연속 반발**(거리 반비례) + **EMA 저역통과 필터**
     (3명+ 밀집 시 A↔B↔C 순환 리밋사이클을 스무딩으로 수렴).
  ② GK 진동 — 라인시프트/볼쏠림을 GK도 받아 골문 앞에서 볼 따라 흔들림(계측: 코너
     경기에서 양 팀 GK가 진동 top). → GK는 볼 좌우 각도만 완만히 커버·전후 고정.
  ③ 세트피스 밀집 — 박스 쇄도 9명이 서로 반발. → 쇄도 선수(strongPullIds)·GK 반발 제외.
  ④ idle 워블 완화(±0.35→0.2, 주기 연장).
  대부분 경기 미세 진동 중앙값 1~2로. 밀집 심한 특정 편성엔 잔여(지표 노이즈 큼 —
  게이트는 median으로 전반 회귀만 포착).
- **공 훅**: FLIGHT_RATIO 0.72→0.88(비행을 delay의 더 많은 비율로 — 프레임당 이동 감소).
  easeOut 유지(advisor: 굴러가는 공 감속이 물리적으로 맞음, easeInOut은 역효과).
