...
- **부르르 떨림 = separation 톱니 진동**: ablation(separatePlayers 끄기)으로 반전 9.5→4.8
  확인 → separation이 절반 유발. 원인: current를 직접 밀어 velocity와 불일치(스프링 당김
  vs 반발 밀기 톱니). 수정: 2패스 리팩터 — 목표(target)에 **연속 반발**(거리 반비례, 하드
  임계 제거) 적용 → 스프링이 밀린 목표로 임계감쇠 수렴. idle 워블도 완화(±0.35→0.2,
  주기 연장). 미세 진동 최악 5.2로(정제 측정).
